...
/**
 * Singular Value Decomposition. <P> For an <i>m</i>&times;<i>n</i> matrix <i>A</i> 
 * with <i>m</i> &ge; <i>n</i>, the
 * singular value decomposition is an <i>m</i>&times;<i>n</i> orthogonal matrix <i>U</i>, 
 * an <i>n</i>&times;<i>n</i>
 * diagonal matrix <i>S</i>, and an <i>n</i>&times;<i>n</i> orthogonal matrix <i>V</i> 
 * so that <i>A = U&sdot;S&sdot;V<sup>*</sup></i>.
 * <p>
 * The singular values, &sigma;[k] = S[k][k], are ordered so that &sigma;[0] &ge;
 * &sigma;[1] &ge; ... &ge; &sigma;[n-1].
 * </p>
 * <p> The singular value decompostion always
...
public class SingularValueDecomposition { // implements java.io.Serializable {
    ...
    * @param A a rectangular matrix with numbers of rows &ge; numbers of columns
    * @throws IllegalArgumentException if number of rows &lt; number of columns
    */
   public SingularValueDecomposition(Matrix A) {
      ...
      if (m < n) {
         throw new IllegalArgumentException("Number of rows "+m+" < number of columns "+n);
      }
      ...
      for (int k = 0; k < Math.max(nct, nrt); k++) {
         if (k < nct) {
            ...
            s[k] = 0;
            for (int i = k; i < m; i++) {
               s[k] = Numbers.hypotenuse(s[k], A_[i][k]);
            }
            if (s[k] != 0.0) {
               if (A_[k][k] < 0.0) {
                  s[k] = -s[k];
         ...
         if (k < nrt) {
            ...
            e[k] = 0;
            for (int i = k + 1; i < n; i++) {
               e[k] = Numbers.hypotenuse(e[k], e[i]);
            }
            if (e[k] != 0.0) {
               if (e[k + 1] < 0.0) {
                  e[k] = -e[k];
      ...
      while (p > 0) {
         ...
         switch (kase) {
            ...
            case 1: {
               ...
               e[p - 2] = 0.0;
               for (int j = p - 2; j >= k; j--) {
                  double t = Numbers.hypotenuse(s[j], f);
                  double cs = s[j] / t;
                  double sn = f / t;
                  s[j] = t;
                  if (j != k) {
            ...
            case 3: {
               ...
               // Chase zeros.
               for (int j = k; j < p - 1; j++) {
                  double t = Numbers.hypotenuse(f, g);
                  double cs = f / t;
                  double sn = g / t;
                  if (j != k) {
                     e[j - 1] = t;
    ...
    * @return the diagonal matrix S of singular values
    */
   public Matrix getS() {
   ...
   public static void main(String[] args) {
      ...
      System.out.println("rank(A)="+A.rank()+", ||A||="+A.norm2());
      System.out.println(A);
      Matrix M;
      M = SVD.getU().times(SVD.getS()).times(SVD.getV().transpose());
      javax.swing.JOptionPane.showMessageDialog(null, "<html>" + M.toHTML("right"));
   }
   // */
}
