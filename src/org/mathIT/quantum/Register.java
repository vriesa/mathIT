...
public class Register {
   ...
   public boolean equals(Object o) {
      ...
      if (real.length != r.real.length) {
         return false;
      }
      if (real.length != r.imaginary.length) {
         return false;
      }
      
      // Compare up to an overall phase, determined by the first nonvanishing amplitude:
      double[] phase = new double[2];
      for (int i = 0; (phase[0] == 0 && phase[1] == 0) && i < real.length; i++) {
         if (abs(r.real[i]) >= ACCURACY || abs(r.imaginary[i]) >= ACCURACY) {
            if (abs(real[i]) < ACCURACY && abs(imaginary[i]) < ACCURACY) {
               return false;
            }
            phase = org.mathIT.numbers.Complex.divide(
               new double[]{r.real[i], r.imaginary[i]}, new double[]{real[i], imaginary[i]}
            );
         }
      }
...
public String toString() {
   ...
   public static void main (String[] args) {
      ...
      //System.out.println("Best number of steps: "+ PI/(4*asin(sqrt(1./power2(n)))));
      //System.out.println("Approximate best number of steps: "+ 7*PI/40*sqrt(power2(n)));
      for (n = 3; n <= 1; n++) {
         register = new Register(n);
         int recommended = groverSteps(register.size);
         //if (recommended != (int) (sqrt(power2(n)/2) * PI/4)) System.out.println("### "+recommended +"!=");
         int needle = 1;
         for (int max = recommended - 1; max <= recommended + 1; max++) {
            register = new Register(n);
            register.qft(power2(n), power2(n));
            for (int i=1; i <= max; i++) {
               register.grover(needle);
            }
            System.out.println("n="+n+", max="+max+", p("+needle+")="+register.real[needle]*register.real[needle]);
            for (int i=1; i <= max; i++) {
               register.inverseGrover(needle);
            }
